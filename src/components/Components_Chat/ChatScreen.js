...
export default class ChatScreen extends Component {
    constructor (props) {
        ...
        if (!this.props.navigation.state.params.channelUrl) {
            console.log('Can not get channel url')
    ...
    componentWillMount () {
        ...
        sendBird.removeChannelHandler(UNIQUE_HANDLER_ID.toString())
    }
    ...
    setupChat = () => {
        ...
        sendBird.GroupChannel.getChannel(this.channelUrl, (channel, error) => {
            if (error) {
                console.log(error);
                console.log('Can not get channel, try again ' + error.message);
    ...
    setupListener = () => {
        ...
        sendBird.addChannelHandler(UNIQUE_HANDLER_ID.toString(), channelHandler)
    };
    ...
    loadHistory = () => {
        ...
        messageListQuery.load(30, true, (messageList, error) => {
            if (error) {
                console.error(error);
                console.log('Can not get history messages')
            } else {
                this.setState({
    ...
    renderItem = ({item, index}) => {
        // Message right (mine)
        if (item.sender === this.currentUser) {
            return (
            ...
            return (
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    {(this.state.arrMessage[index - 1] &&
                        this.state.arrMessage[index - 1].sender === this.currentUser) ||
                    index === 0 ? (
                        <Image
